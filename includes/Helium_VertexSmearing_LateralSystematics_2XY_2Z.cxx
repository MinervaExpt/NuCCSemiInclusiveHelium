...
template <class T>
std::vector<T*> GetHELIUMVertexSmearingShiftSystematics(
  ...
  std::vector<T*> ret;
  std::cout
      << "Vertex Helium Smearing for a shifts in 4 universe (-/+)(deltaR) =  ("
      << m_Uncertainty_R << "[mm]) and (-/+)(deltaZ) = ()" << m_Uncertainty_Z
      << "[mm])" << std::endl;
  ret.push_back(
      new HeliumVertexSmearingRShiftUniverse<T>(chain, -1., m_Uncertainty_R));
  ret.push_back(
      new HeliumVertexSmearingRShiftUniverse<T>(chain, 1., m_Uncertainty_R));
  ret.push_back(
      new HeliumVertexSmearingZShiftUniverse<T>(chain, -1., m_Uncertainty_Z));
  ret.push_back(
      new HeliumVertexSmearingZShiftUniverse<T>(chain, 1., m_Uncertainty_Z));
...
std::string HeliumVertexSmearingRShiftUniverse<T>::ShortName() const {
  // std::string universe_name = (T::m_nsigma > 0 ) ? "VertexSmearingR_Pos" :
  // "VertexSmearingR_Neg";
...
template <typename T>
std::string HeliumVertexSmearingRShiftUniverse<T>::LatexName() const {
  return "Vertex R Smearing";
}

template <typename T>
std::string HeliumVertexSmearingZShiftUniverse<T>::ShortName() const {
  // std::string universe_name = (T::m_nsigma > 0 ) ? "VertexSmearingZ_Pos" :
  // "VertexSmearingZ_Neg";
...
template <typename T>
std::string HeliumVertexSmearingZShiftUniverse<T>::LatexName() const {
