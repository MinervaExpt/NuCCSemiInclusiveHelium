...
PlayList_INFO(ME_helium_Playlists playlist, std::string RootPath ,bool is_mc, bool IsTRUTH, bool Stats_only ){
  m_IsTRUTH = IsTRUTH;
  m_ismc = is_mc;
  m_IsStatsOnly=Stats_only;
  m_playlist = playlist;
  ...
  m_playlist_title = GetPlaylist_Name(playlist);
  m_Helium_status_title  =GetFULLorEmpty( m_Helium_status);
  m_Root_Pathway = RootPath;

  TFILE_PLAYLIST = new TFile(m_Root_Pathway.c_str());
  Set_Pot();
}

PlayList_INFO( std::string RootPath ,bool is_mc, bool IsTRUTH, bool Stats_only ){
  m_IsTRUTH = IsTRUTH;
  m_ismc = is_mc;
  m_IsStatsOnly=Stats_only;
  m_Root_Pathway = RootPath;
...
void Compare_Pot_MapStatusList(Pot_MapList &Map1, Pot_MapList &Map2  , const char* name_var)
{
  std::cout<<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
  std::cout<<"~~~~~~~~~Comparing Two Maps: for "<< name_var << "~~~~~~~~~~~~~~~~" << std::endl;
  std::cout<<"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
  if(CompareMap_ME_helium_PlaylistsKeys(Map1, Map2 )) {

    std::cout<< " Playlist , "<< name_var << ",Map1, Map2, ratio" <<  std::endl;
    ...
    for(dog = Map1.begin(), cat = Map2.begin(); dog != Map1.end(); ++dog, ++cat)
    {
      double map1 = dog->second;
      double map2 = cat->second;
      double ratio = map1 / map2;
      std::cout<< GetPlaylist_Name(dog->first) <<" , "<< name_var << ", "<< map1 <<"," << map2 << ","  <<ratio<<std::endl;
    }
  }
  else{
    std::cout<<"Maps don't have Same Playlist Keys can't compare" << std::endl;
  }
...
//==============================================================================
// Predicate to erase items from a container (map or vect).
// I use it like: "if item in container satisfies property X then remove it
// Example
