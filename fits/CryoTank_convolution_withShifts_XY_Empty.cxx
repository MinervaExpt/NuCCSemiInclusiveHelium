...
MnvPlotter *mnv_plotter = new MnvPlotter();
 ...
 outputTxtfile.open("FIT_XY_Empty_run3_new.txt");
//time_t now = time(0);
...
Convolution_Tool_TotalPlaylist.init_All_DataEmptyHists();
...
Convolution_Tool_TotalPlaylist.Plot_regions_XYFit_Empty( xyMape , 0, 0, "test" );
...
double EmptyChisqrt_Top1 = 3117.62;
double EmptyChisqrt_Top2 = 3130.82;
double EmptyChisqrt_Top3 = 3131.62;
double EmptyChisqrt_Top4 = 3131.69;
double EmptyChisqrt_Top5 = 3132.74;
Index4 EmptyTop1index{2 ,3, 2, 2 };
Index4 EmptyTop2index{2 ,4, 3, 4};
Index4 EmptyTop3index{1 ,1, 3, 1 };
Index4 EmptyTop4index{1 ,4, 2, 2};
Index4 EmptyTop5index{3 ,2, 3, 0 };

double EmptyChisqrt_Top1R = 2969.2 ;
double EmptyChisqrt_Top2R = 2970.04;
double EmptyChisqrt_Top3R = 2978.6;
double EmptyChisqrt_Top4R = 2978.63;
double EmptyChisqrt_Top5R = 2978.87;
Index4 EmptyTop1indexR{2 ,0, 4, 0};
Index4 EmptyTop2indexR{3 ,0, 4, 0 };
Index4 EmptyTop3indexR{3 ,0, 5, 0 };
Index4 EmptyTop4indexR{3 ,1, 5, 0 };
Index4 EmptyTop5indexR{1 ,1, 3, 0};

std::map<double , Index4> EmptyChisqrtMap{{EmptyChisqrt_Top1 ,EmptyTop1index},{EmptyChisqrt_Top2 ,EmptyTop2index}, {EmptyChisqrt_Top3 ,EmptyTop3index}, {EmptyChisqrt_Top4 ,EmptyTop4index}, {EmptyChisqrt_Top5 ,EmptyTop5index}};
std::map<double , Index4> EmptyChisqrtMapR{{EmptyChisqrt_Top1R ,EmptyTop1indexR},{EmptyChisqrt_Top2R ,EmptyTop2indexR}, {EmptyChisqrt_Top3R ,EmptyTop3indexR}, {EmptyChisqrt_Top4R ,EmptyTop4indexR}, {EmptyChisqrt_Top5R ,EmptyTop5indexR}};
...
Convolution_Tool_TotalPlaylist.initEMPTYONLY_RECO_Convolution_Material_Regions_Hists_PlaylistCombinedMC( dog->first.i,  dog->first.j,  dog->first.k,  dog->first.m );
...
if(Chisqrt_X_Y_Z < EmptyChisqrtMap.rbegin()->first ){

EmptyChisqrtMap.insert(std::pair<double , Index4 >(Chisqrt_X_Y_Z, CurrentIndex));


EmptyChisqrtMap.erase(EmptyChisqrtMap.rbegin()->first);


}


if( ChisqrtR_Z < EmptyChisqrtMapR.rbegin()->first ){

EmptyChisqrtMapR.insert(std::pair<double , Index4 >( ChisqrtR_Z, CurrentIndex));
EmptyChisqrtMapR.erase(EmptyChisqrtMapR.rbegin()->first);
...
for(auto cat = EmptyChisqrtMap.begin(); cat != EmptyChisqrtMap.end();++cat){

std::cout<< "EmptyChisqrtMap:cat->first = " << cat->first<< " (i,j,k,m) =  ("<<cat->second.i << " ,"<< cat->second.j << ", "<< cat->second.k << ", "<< cat->second.m<< " )"<< std::endl;

}
for(auto cat = EmptyChisqrtMapR.begin(); cat != EmptyChisqrtMapR.end();++cat){

std::cout<<"EmptyChisqrtMapR: cat->first = " << cat->first<< " (i,j,k,m) =  ("<<cat->second.i << " ,"<< cat->second.j << ", "<< cat->second.k << ", "<< cat->second.m<< " )"<< std::endl;
