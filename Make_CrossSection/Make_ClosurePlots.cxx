...
auto PlaylistME_CrossSection_Path = "/minerva/app/users/cnguyen/cmtuser/Minerva_v22r1p1Helium_GIT/NuCCSemiInclusiveHelium/Make_CrossSection/CrossSection_closureTest.root";

auto PlaylistME_CrossSection_GENIE = "/minerva/app/users/cnguyen/cmtuser/Minerva_v22r1p1Helium_GIT/opt/build_GENIE_Extract_new/apps/GENIEXSecExtract_Helium_CCSemi_ME1L.root";
...
sprintf(text_title_pdf1, "PLOTS_%s_ClosureTest.pdf(","" );
can -> Print(text_title_pdf1);
sprintf(text_title_pdf2, "PLOTS_%s_ClosureTest.pdf","" );
sprintf(text_title_pdf3, "PLOTS_%s_ClosureTest.pdf)","" );
sprintf(text_title_pdf4, "PLOTS_%s_ClosureTest","" );
...
TFile *TFile_CrossSection_Genie = new TFile(PlaylistME_CrossSection_GENIE);

/// Check Event Rate


TH1D* h_inhouse_Eventrate = (TH1D*)TFile_CrossSection->Get("h_MuonPT_Full_effDom");
TH1D* h_inhouse_CrossSection = (TH1D*)TFile_CrossSection->Get("h_MuonPT_TrueCrossSection");

TH1D* h_Genie_Eventrate = (TH1D*)TFile_CrossSection_Genie->Get("ccheliumPT_xsec_evRate");
TH1D* h_Genie_CrossSection = (TH1D*)TFile_CrossSection_Genie->Get("ccheliumPT_xsec");

std::string crossSection_y_axis = "#frac{d#sigma}{d" + GetMuonVar_crosssection(kP_T) + "} (10^{-38} cm^{2} / GeV / ^{4}He)";
char Y_axis_char[crossSection_y_axis.length()+1];
strcpy(Y_axis_char,crossSection_y_axis.c_str());


MnvH1D *h_inhouse_CrossSection_clone = (PlotUtils::MnvH1D*)h_inhouse_CrossSection->Clone("h_inhouse_CrossSection_clone");
MnvH1D *h_Genie_CrossSection_clone = (PlotUtils::MnvH1D*)h_Genie_CrossSection->Clone("h_Genie_CrossSection_clone");


DrawCVAndError_From2HIST(h_inhouse_CrossSection_clone, "Inhouse cross section" ,h_Genie_CrossSection_clone, "Genie Extractor", "Comparing Genie vs. inhouse crossection" ,"P_{T} [GeV]", Y_axis_char,
      pdf_CV, true, false , false , false, -99, false);

h_Genie_Eventrate->Divide(h_Genie_Eventrate,h_inhouse_Eventrate);
h_Genie_CrossSection->Divide(h_Genie_CrossSection,h_inhouse_CrossSection);

MnvH1D *h_Genie_Eventrate_clone_Ratio = (PlotUtils::MnvH1D*)h_Genie_Eventrate->Clone("h_Genie_Eventrate_clone_Ratio");
MnvH1D *h_Genie_CrossSection_clone_Ratio = (PlotUtils::MnvH1D*)h_Genie_CrossSection->Clone("h_Genie_CrossSection_clone_Ratio");


//DrawCVAndError_FromHIST(h_Genie_Eventrate_clone_Ratio,"Ratio for EventRate #frac{GENIE}{inhouse}","P_{T}","Ratio  #frac{GENIE Event rate}{inhouse Event rate} ",  pdf_CV, true);
//DrawCVAndError_FromHIST(h_Genie_CrossSection_clone_Ratio,"Ratio for EventRate #frac{GENIE cross section}{inhouse cross section}","P_{T}","Ratio  #frac{GENIE cross section}{inhouse cross section} ",  pdf_CV, true);

 DrawCVAndError_FromHIST(false , h_Genie_Eventrate_clone_Ratio, "Ratio for EventRate #frac{GENIE}{inhouse}" ,"P_{T} [GeV]","Ratio  #frac{GENIE Event rate}{inhouse Event rate}" ,
  pdf_CV,true , false, true,  .998,  1.002);

  DrawCVAndError_FromHIST(false , h_Genie_CrossSection_clone_Ratio, "Cross Section Ratio   #frac{d#sigma}{d P_{T}}   #frac{GENIE }{inhouse}" ,"P_{T} [GeV]","Ratio #frac{d#sigma}{d P_{T}}   #frac{GENIE }{inhouse}" ,
   pdf_CV,true , false, true,  1.0025,  1.003);

   DrawCVAndError_FromHIST(false , h_Genie_CrossSection_clone_Ratio, "Cross Section Ratio #frac{d#sigma}{d P_{T}}  #frac{GENIE}{inhouse}" ,"P_{T} [GeV]","Ratio  #frac{GENIE }{inhouse}" ,
    pdf_CV,true , false, true,  .99,  1.01);
